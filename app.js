(() => {
  const CLIPCONTROL_FRONTEND_VERSION = "4.4.1-clean-push-core";
  window.CLIPCONTROL_FRONTEND_VERSION = CLIPCONTROL_FRONTEND_VERSION;
  document.documentElement.dataset.clipcontrolUi = "4.4.1-clean-push-core";
  "use strict";
  ...
  const state = {
    ...
    nativePushLastSync: 0,
  };

  function nativeBridge() {
    try { return window.ClipControlNative || null; } catch (_) { return null; }
  }

  function nativePushSnapshot() {
    const bridge = nativeBridge();
    if (!bridge) return null;
    try {
      const isNative = typeof bridge.isNativeApp === "function" ? bridge.isNativeApp() : false;
      if (!isNative) return null;
      const token = typeof bridge.fcmToken === "function" ? String(bridge.fcmToken() || "").trim() : "";
      const preferencesRaw = typeof bridge.notificationPreferences === "function" ? bridge.notificationPreferences() : "{}";
      let preferences = {};
      try { preferences = JSON.parse(preferencesRaw || "{}"); } catch (_) {}
      return {
        token,
        device_id: typeof bridge.deviceId === "function" ? String(bridge.deviceId() || "") : "",
        device_name: typeof bridge.deviceName === "function" ? String(bridge.deviceName() || "Android") : "Android",
        app_version: typeof bridge.appVersion === "function" ? String(bridge.appVersion() || "") : "",
        fcm_configured: typeof bridge.fcmConfigured === "function" ? Boolean(bridge.fcmConfigured()) : Boolean(token),
        preferences: {
          urgent: preferences.urgent !== false,
          normal: preferences.normal !== false,
          announcement: preferences.announcement !== false,
          promotions: preferences.promotions !== false,
        },
      };
    } catch (error) {
      console.warn("Native push snapshot no disponible", error);
      return null;
    }
  }

  async function registerNativePushDevice(force=false) {
    if (!state.supabase || !state.profile?.id) return false;
    const snapshot = nativePushSnapshot();
    if (!snapshot?.token) return false;
    const now = Date.now();
    if (!force && now - Number(state.nativePushLastSync || 0) < 60_000) return true;
    try {
      const { error } = await state.supabase.rpc("register_my_push_device", {
        p_token: snapshot.token,
        p_platform: "android",
        p_device_id: snapshot.device_id || null,
        p_device_name: snapshot.device_name || "Android",
        p_app_version: snapshot.app_version || null,
        p_preferences: snapshot.preferences,
      });
      if (error) throw error;
      state.nativePushLastSync = now;
      return true;
    } catch (error) {
      // La web debe seguir funcionando incluso antes de instalar la migración push.
      console.warn("No se pudo registrar el dispositivo push", error);
      return false;
    }
  }

  async function unregisterNativePushDevice() {
    if (!state.supabase) return;
    const snapshot = nativePushSnapshot();
    if (!snapshot?.token) return;
    try { await state.supabase.rpc("unregister_my_push_device", { p_token: snapshot.token }); }
    catch (error) { console.warn("No se pudo desregistrar push", error); }
  }

  function nativeNotificationPreferencesCard() {
    const snapshot = nativePushSnapshot();
    if (!snapshot) return "";
    const p = snapshot.preferences || {};
    const row = (key,label,help) => `<label class="checkbox-label full"><input type="checkbox" data-native-notification-pref="${key}" ${p[key]!==false?"checked":""}> <span><b>${label}</b><small style="display:block;margin-top:3px">${help}</small></span></label>`;
    return `<section class="card native-notifications-card"><div class="card-head"><div><h2>Notificaciones del celular</h2><p>Preferencias guardadas en este dispositivo Android.</p></div><span class="pill ${snapshot.token?"pill-green":"pill-yellow"}">${snapshot.token?"Push listo":snapshot.fcm_configured?"Registrando":"Firebase pendiente"}</span></div><div class="form-grid compact-form">${row("urgent","Urgentes","Sonido, vibración fuerte y aviso destacado.")}${row("normal","Avisos normales","Sonido estándar para recordatorios y cambios.")}${row("announcement","Comunicados","Avisos suaves, sin interrupción fuerte.")}${row("promotions","Promociones","Contenido promocional silencioso.")}</div></section>`;
  }

  function bindNativeNotificationPreferences(root=document) {
    root.querySelectorAll?.("[data-native-notification-pref]").forEach(input => {
      input.addEventListener("change", async () => {
        const bridge = nativeBridge();
        try { bridge?.setNotificationPreference?.(input.dataset.nativeNotificationPref, input.checked); } catch (_) {}
        await registerNativePushDevice(true);
      });
    });
  }

  window.addEventListener("clipcontrol-fcm-token", () => { registerNativePushDevice(true).catch(()=>{}); });
  ...
  function accessPaymentStatusLabel(status) {
    return ({
  ...
  function accessPaymentStatusClass(status) {
  ...
  function accessQrPublicUrl(path) {
  ...
  function normalizeAccessPaymentRow(value) {
  ...
  async function refreshMyAccessPayment() {
    if (state.profile?.role !== "clipper") {
      state.accessPayment = { can_upload: true, fee: null, settings: null };
      return state.accessPayment;
    }
    try {
      const result = await query(state.supabase.rpc("clipcontrol_my_access_payment_v320"));
      state.accessPayment = normalizeAccessPaymentRow(result);
    } catch (error) {
      ...
      if (/clipcontrol_my_access_payment_v320|PGRST202|does not exist|Could not find the function/i.test(msg)) {
        ...
        state.accessPayment = { can_upload: true, fee: null, settings: null, setup_missing: true };
      } else {
        throw error;
      }
    }
  ...
  function clipperUploadEnabled() {
    if (state.profile?.role !== "clipper") return true;
    return state.accessPayment?.can_upload !== false;
  }

  function clipperAccessPaymentMarkup() {
    if (state.profile?.role !== "clipper") return "";
    const access = state.accessPayment;
    ...
    if (!access.fee) {
      if (access.can_upload !== false) return "";
      const qrUrl = accessQrPublicUrl(settings.qr_path);
    ...
    return `<section class="access-payment-gate-v320 ${accessPaymentStatusClass(status)}">
      <div class="access-payment-copy-v320">
        <span class="section-eyebrow">ACCESO A LA PLATAFORMA</span>
        <h3>${accessPaymentStatusLabel(status)}</h3>
        ...
        <div class="access-payment-meta-v320">
          ...
          <span><small>Estado</small><b>${esc(accessPaymentStatusLabel(status))}</b></span>
        </div>
  ...
  function bindClipperAccessPaymentActions(root = document) {
    $$("[data-upload-access-proof]", root).forEach(button => button.addEventListener("click", openAccessPaymentProofModal));
  }

  async function openAccessPaymentProofModal() {
    const access = await refreshMyAccessPayment();
    const fee = access?.fee;
    ...
    openModal(`<div class="modal-head"><div><h2>Comprobante de pago</h2><p>Sube una captura clara. Administración debe aprobarla para habilitar tus clips.</p></div><button id="accessProofX" class="modal-close">×</button></div>
      <form id="accessProofForm"><div class="modal-body">
        <div class="access-proof-summary-v320"><div><span>Monto</span><b>${money(fee.amount || 0)}</b></div><div><span>Estado</span><b>${esc(accessPaymentStatusLabel(fee.status))}</b></div></div>
  ...
  async function fetchAdminAccessFees() {
    try {
  ...
  function adminAccessFeeBadge(fee) {
    if (!fee?.fee_id) return `<span class="access-fee-admin-badge-v320 is-neutral">Sin cobro</span>`;
    return `<span class="access-fee-admin-badge-v320 ${accessPaymentStatusClass(fee.fee_status)}">${esc(accessPaymentStatusLabel(fee.fee_status))}</span>`;
  }

  async function fetchAccessPaymentSettings() {
    try {
  ...
  async function openAccessPaymentSettingsAdmin() {
    const settings = await fetchAccessPaymentSettings();
    const qrUrl = accessQrPublicUrl(settings.qr_path);
  ...
  async function openAccessFeeAdminModal(userId) {
    ...
    openModal(`<div class="modal-head"><div><h2>Cobro de acceso</h2><p>${esc(user.names ? `${user.names} ${user.surnames || ""}`.trim() : `@${user.username}`)} · @${esc(user.username || "")}</p></div><button id="accessFeeX" class="modal-close">×</button></div>
      <div class="modal-body">
        <div class="admin-access-fee-state-v320">
          <div><span>Estado actual</span>${adminAccessFeeBadge(fee)}</div>
  ...
  function metricReviewBucket(video = {}) {
    if (Number(video.manual_override_count || 0) > 0) return "manual";
    return metricBucket(video);
  }

  function metricReviewLabel(bucket) {
    return bucket === "manual" ? "Manual activa" : metricBucketLabel(bucket);
  }

  function metricReviewBadge(video = {}) {
    const bucket = metricReviewBucket(video);
    return `<span class="metric-quality metric-quality-${bucket}"><i></i>${metricReviewLabel(bucket)}</span>`;
  }

  async function loadMetricReviewMeta(data) {
    ...
    for (const video of data.videos || []) {
      ...
      video.metric_review_bucket = metricReviewBucket(video);
    }
    return data;
  }
  ...
  function bindAdminVideoCards() {
    ...
    $$('[data-review-metrics-v430]').forEach(button => button.addEventListener("click", () => openMetricReview(button.dataset.reviewMetricsV430)));
  }
  ...
  async function openMetricReview(videoId) {
    showLoading(true);
    try {
      let data = await loadMetricReviewMeta(await loadAdminVideoCenterData(null,false));
  ...
  async function renderMetricInbox(force = false) {
    ...
    const data = await loadMetricReviewMeta(await loadAdminVideoCenterData(null, force));
    ...
    const baseIssues = data.videos.filter(video => metricReviewBucket(video) !== "ok");
    ...
    const errors = baseIssues.filter(video=>metricReviewBucket(video)==="error").length;
    const partial = baseIssues.filter(video=>metricReviewBucket(video)==="partial").length;
    const pending = baseIssues.filter(video=>["pending","syncing"].includes(metricReviewBucket(video))).length;
    const manual = baseIssues.filter(video=>metricReviewBucket(video)==="manual").length;
  ...
  async function usePublicYoutubeVideo(url) {
    ...
    if (!clipperUploadEnabled()) return toast("Tu acceso para subir clips está bloqueado por un pago pendiente.", "error");
      ...
      <div class="modal-foot"><button type="button" id="detectAgain" class="btn btn-secondary">↻ Detectar ahora</button><div class="actions"><button type="button" id="editCancel" class="btn btn-ghost">Cancelar</button><button class="btn btn-primary">Guardar cambios</button></div></div></form>`, "small", (layer) => {
        ...
        $("#editVideoForm", layer).addEventListener("submit", async (event) => {
          ...
          try {
            ...
            if (!adminMode && !clipperUploadEnabled()) throw new Error("Tu acceso para modificar clips está bloqueado por un pago pendiente.");
  ...
  async function requestAvatarAnnouncement(userIds) {
  ...
  async function renderAdminClippers() {
    ...
    if(toolbar&&!$("#clipperProfileFilterV400")){
      ...
      $("#requestPhotosV400")?.addEventListener("click",async()=>{if(!confirm(`¿Solicitar foto a ${missingPhoto.length} clipero(s) que aún no tienen avatar?`))return;try{await requestAvatarAnnouncement(missingPhoto.map(u=>u.user_id));}catch(error){toast(errorMessage(error),"error");}});
    }
    $$("[data-open-user]").forEach(open=>{
      ...
      btn.addEventListener("click",async()=>{try{await requestAvatarAnnouncement([uid]);}catch(error){toast(errorMessage(error),"error");}});
    });
  }

  function clipperVideoDateMatch(video,mode){
  ...
  function openClipperVideoFilters(onApply){
    ...
    openModal(`<div class="modal-head"><div><span class="section-eyebrow">FILTROS</span><h2>Filtrar videos</h2></div><button class="modal-close" data-video-filter-close-v400 aria-label="Cerrar">×</button></div><div class="modal-body"><div class="form-grid compact-form"><label>Plataforma<select id="clipVideoPlatformSheetV400"><option value="all">Todas</option>${Object.keys(PLATFORMS).map(p=>`<option value="${p}" ${f.platform===p?"selected":""}>${platformLabel(p)}</option>`).join("")}</select></label><label>Cuenta<select id="clipVideoAccountSheetV400"><option value="all">Todas</option>${activeAccounts().map(a=>`<option value="${a.id}" ${f.account===a.id?"selected":""}>${esc(a.account_name)}</option>`).join("")}</select></label><label>Estado<select id="clipVideoStatusSheetV400"><option value="all">Todos</option>${["ok","partial","error","syncing","pending"].map(x=>`<option value="${x}" ${f.status===x?"selected":""}>${metricBucketLabel(x)}</option>`).join("")}</select></label><label>Fecha<select id="clipVideoDateSheetV400"><option value="all" ${f.date==="all"?"selected":""}>Todo el período</option><option value="today" ${f.date==="today"?"selected":""}>Últimas 24 h</option><option value="48h" ${f.date==="48h"?"selected":""}>Últimas 48 h</option></select></label></div></div><div class="modal-foot"><button class="btn btn-ghost" data-video-filter-reset-v400>Limpiar</button><button class="btn btn-primary" data-video-filter-apply-v400>Aplicar filtros</button></div>`,"small",layer=>{
      ...
      $("[data-video-filter-reset-v400]",layer)?.addEventListener("click",()=>{f.platform="all";f.account="all";f.status="all";f.date="all";setLocal("clipper_video_filters",f);closeModal();onApply();});
      $("[data-video-filter-apply-v400]",layer)?.addEventListener("click",()=>{f.platform=$("#clipVideoPlatformSheetV400",layer).value;f.account=$("#clipVideoAccountSheetV400",layer).value;f.status=$("#clipVideoStatusSheetV400",layer).value;f.date=$("#clipVideoDateSheetV400",layer).value;setLocal("clipper_video_filters",f);closeModal();onApply();});
    });
  }

  function renderClipperVideos() {
    setHeader("Videos",periodRangeLabel(state.currentSummary));
    const editable=reportEditable(state.currentSummary)&&clipperUploadEnabled();
    const saved=getLocal("clipper_video_filters",{})||{};
    ...
    const visible=state.videos.filter(video=>{
      ...
      return (!f.search||hay.includes(f.search.toLowerCase()))&&(f.platform==="all"||video.platform===f.platform)&&(f.account==="all"||video.account_id===f.account)&&(f.status==="all"||metricBucket(video)===f.status)&&clipperVideoDateMatch(video,f.date);
  ...
  async function saveOwnProfile(event) {
    ...
    await saveProfile({ names:f.names ?? p.names, surnames:f.surnames ?? p.surnames, phone:f.phone ?? p.phone, primary_social_url:f.primary_social_url ?? p.primary_social_url, payment_method:f.payment_method ?? p.payment_method, payment_account:f.payment_account ?? p.payment_account, payment_holder:f.payment_holder ?? p.payment_holder });
  }
  ...
  function renderAdminRulesBase(profile, rules) {
  ...
  async function login(event) {
    ...
    async function refreshAnnouncementBadge(force=false) {
    const badge=$("#noticeBadge");
    if(!badge||!state.profile)return;
    const now=Date.now();
    if(!force&&state._announcementBadgeLoadedAt&&now-state._announcementBadgeLoadedAt<CC_PERF.announcementTtlMs)return;
    if(state._announcementBadgeInflight)return state._announcementBadgeInflight;
    state._announcementBadgeInflight=(async()=>{
      try{
        ...
        state._announcementBadgeLoadedAt=Date.now();
      }catch(_){badge.classList.add("hidden");}
    })();
    try{await state._announcementBadgeInflight;}finally{state._announcementBadgeInflight=null;}
  }
  ...
  async function renderAdminSettingsCore() {
    ...
    $$('[data-settings-tab]').forEach(b=>b.addEventListener("click",()=>{state.settingsTab=b.dataset.settingsTab;renderAdminSettingsBase();}));
    const saveSettings=async(payload,msg)=>{try{await query(state.supabase.from("app_settings").update(payload).eq("id",1));toast(msg,"success");await loadGlobalContext();await renderAdminSettingsBase();}catch(error){toast(errorMessage(error),"error");}};
  ...
  // ================================================================
  // CLIPCONTROL · CLIPPER DATA + REALTIME UI
  // ================================================================
  ...
  function setLiveStatus(mode = "connected", text = "En vivo") {
    ...
    $("#menuBtn")?.addEventListener("click",()=>{if(window.innerWidth<=1180)openSidebar();else $("#sidebar")?.classList.toggle("open");});
    $("#themeBtn")?.addEventListener("click",toggleTheme);
    $("#noticeBtn")?.addEventListener("click",openNoticeInbox);

    document.addEventListener("visibilitychange",()=>{
      if(document.visibilityState!=="visible") return;
      // En móvil no lanzamos sincronizaciones de métricas al volver a primer plano.
      // Solo marcamos caches como potencialmente vencidos y renovamos presencia sin bloquear UI.
      touchPresence(true);
      if(state._backgroundedAt && Date.now()-state._backgroundedAt>60_000){
        state._globalContextLoadedAt=0;
        state._clipperDataLoadedAt=0;
      }
      state._backgroundedAt=0;
      refreshAnnouncementBadge(false);
    });
    window.addEventListener("pagehide",()=>{state._backgroundedAt=Date.now();},{passive:true});

    window.addEventListener("keydown",event=>{
      if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="k"&&["admin","superadmin"].includes(state.profile?.role)){
        event.preventDefault();openCommandCenter();
      }
    });
    window.matchMedia?.("(prefers-color-scheme: dark)")?.addEventListener?.("change",()=>{
      if((localStorage.getItem("clipcontrol_theme_v21")||state.settings?.default_theme||"system")==="system")applyThemeMode("system");
    });
  }
  ...
  async function seedLiveMetricCache() {
    ...
    if(type==="INSERT"){
      if(row.id)state.liveVideoMetricCache.set(row.id,metricTuple(row));
      return true;
    }
    if(type==="DELETE"){
      if(old.id)state.liveVideoMetricCache.delete(old.id);
      return true;
    }
    if(type!=="UPDATE"||!row.id)return false;
    const next=metricTuple(row);
    const previous=state.liveVideoMetricCache.get(row.id);
    state.liveVideoMetricCache.set(row.id,next);
    // Si aún no terminó el seed inicial, preferimos refrescar una vez antes que perder un cambio real.
  ...
  async function renderLiveSilently(renderer) {
    ...
    state.liveMetricCacheReady=false;
    // El seed puede ser pesado para administradores. Se hace en segundo plano y no bloquea el primer render.
    seedLiveMetricCache().catch(error=>console.warn("No se pudo preparar cache LIVE",error));
  ...
  async function logout() {
    ...
    try{await touchPresence(true);await unregisterNativePushDevice();await state.supabase.auth.signOut();}
  ...
  function avatarProfileForUser(userId, fallback = {}) {
  ...
  function clipperIdentityMarkup(report, size = "sm") {
    const profile = avatarProfileForUser(report?.user_id, {
  ...
  function topVideoOwnerBadge(video) {
    const fallback = {
      names: video?.clipper_name || video?.username || "Clipero",
      username: video?.username || "",
      role: "clipper"
    };
    const profile = avatarProfileForUser(video?.user_id, fallback);
  ...
  async function init() {
    ...
    try{
      ...
      showApp();
      const rememberedPage=getLocal("last_page","dashboard");
      ...
      // El primer panel ya está visible. Todo lo secundario arranca después y no bloquea el WebView.
      requestAnimationFrame(()=>setTimeout(()=>{
        startPresenceHeartbeat();
        startLiveRealtime().catch(error=>console.warn("Realtime no disponible",error));
        refreshAnnouncementBadge(false);
        registerNativePushDevice(false).catch(()=>{});
        if(state.profile?.role==="clipper"&&!profileComplete(state.profile))openProfileModal(true);
        else showMandatoryAnnouncements().catch(()=>{});
      },80));
    }catch(error){
      toast(errorMessage(error),"error");
      await state.supabase.auth.signOut();
    }finally{
      if(!firstPaintDone)showLoading(false);
    }
  }
  ...
  async function renderAdminClippersBase() {
    ...
    const [overview,profiles,accessFees,distributionSettings] = await Promise.all([
      ...
      fetchAdminAccessFees(),
      fetchPaymentDistributionSettings(),
    ]);
    ...
    const cards = visible.map(u => {
      ...
      const feeBlock = u.role === "clipper" ? `<div class="access-fee-card-v320">
        ...
        <div>${adminAccessFeeBadge(fee)}<small>${fee?.can_upload===false?"Subida bloqueada":"Subida habilitada"}</small></div>
      </div>` : "";
    ...
    $("#contributionAdminFormV360")?.addEventListener("submit",async event=>{
      ...
      try {
        await savePaymentDistributionRecipient(form.recipient_user_id);
        state.paymentDistributionSettingsV360 = await fetchPaymentDistributionSettings();
    ...
    $("#configureAccessQrBtnV320")?.addEventListener("click",openAccessPaymentSettingsAdmin);
    $("#requestAllPayBtn")?.addEventListener("click",async()=>{if(!confirm("¿Solicitar datos de pago a todos los cliperos activos que aún no los registraron?"))return;try{const count=await query(state.supabase.rpc("admin_request_payment_data_all"));toast(`Solicitud activada para ${count} clipero(s)`,"success");await renderAdminClippersBase();}catch(error){toast(errorMessage(error),"error");}});
    $$('[data-access-filter]').forEach(b=>b.addEventListener("click",()=>{state.accessRoleFilter=b.dataset.accessFilter;renderAdminClippersBase();}));
    $("#activityFilter")?.addEventListener("change",e=>{state.accessActivityFilter=e.target.value;renderAdminClippersBase();});
    ...
    $$('[data-access-fee-user]').forEach(b=>b.addEventListener("click",()=>openAccessFeeAdminModal(b.dataset.accessFeeUser)));
  ...
  function adminReportsTable(reports) {
    ...
    const rows = reports.map(r => `<tr class="report-row-v224 row-${statusClass(r.status)}"><td>${clipperIdentityMarkup(r,"sm")}</td><td><div class="report-number-v224"><b>${r.video_count}</b><small>${r.account_count} cuenta${Number(r.account_count)===1?"":"s"}</small></div></td><td><div class="report-metric-main metric-main-v224">${uiIcon("eye",14)}<b>${num(r.total_views)}</b></div><div class="report-like-line metric-like-v224">${uiIcon("heart",12)} ${num(r.total_likes||0)}</div></td><td>${reportPlatformProgressMarkup(r.report_id,r.user_id)}</td><td><b class="report-pay-v224">${money(reportFinalTotal(r))}</b><small class="pay-sum-note">Pago final del período</small></td><td>${statusBadge(r.status)}</td><td>${actionMarkup(r)}</td></tr>`).join("");
    const cards = reports.map(r => `<article class="mobile-report-card-v224 row-${statusClass(r.status)}"><div class="mobile-report-head-v224">${clipperIdentityMarkup(r,"sm")}${statusBadge(r.status)}</div><div class="mobile-report-stats-v224"><div><span>Videos</span><b>${r.video_count}</b></div><div><span>Vistas</span><b>${num(r.total_views)}</b></div><div><span>Likes</span><b>${num(r.total_likes||0)}</b></div><div><span>Pago</span><b>${money(reportFinalTotal(r))}</b></div></div><div class="mobile-network-breakdown"><span>Avance por redes</span>${reportPlatformProgressMarkup(r.report_id,r.user_id)}</div>${actionMarkup(r)}</article>`).join("");
  ...
  async function exportWeeklyExcel(reports) {
    ...
    try {
      ...
      const exportPaymentBundle = buildFrontendPaymentBundle(reports || [], videoRows || [], socialAccounts || [], paymentRules || [], exportSettings || {});
      const accountPaymentRows = exportPaymentBundle.accountRows;
      const paymentTotalsRows = exportPaymentBundle.totals;
      const paymentRuleV330 = frontendPaymentRuleSummary(paymentRules || [], exportSettings || {});
      ...
      const exportDistribution = buildPaymentDistribution(reports || [], distributionSettings || {});
      ...
      const reportTotal = (report) => roundMoney(exportDistribution.byReport?.[report?.report_id]?.final ?? paymentTotalMap[report?.report_id]?.total_pay ?? 0);
      ...
      const summaryRows = [
        ...
        ...platformOrder.map((platform) => {
          ...
          const platformRule = frontendPlatformPaymentRule(platform, paymentRules || [], exportSettings || {});
          return [
...
<Styles>
  ...
  async function ensureExportLibs() {
    await Promise.all([
      ...
      loadScriptOnce("https://unpkg.com/jspdf@2.5.2/dist/jspdf.umd.min.js"),
    ]);
  }

  

  function alias(report) {
    const raw = String(report.social_alias || report.username || "").trim();
    return raw && !raw.startsWith("@") ? `@${raw}` : raw || "—";
  }

  function avatarCircle(url, label) {
  ...
  async function buildVisualReportData(reports) {
    const settings = await fetchPaymentDistributionSettings();
    const distribution = buildPaymentDistribution(reports || [], settings);
    ...
    const allRows = (reports || []).map(report => ({
      ...
      final: roundMoney(reportFinalTotal(report)),
    ...
    const total = roundMoney(rows.reduce((sum, r) => sum + r.final, 0));
    return { rows, totals: { people: allRows.length, withPay, withoutPay, total }, distribution };
  }

  

  async function withVisualCapture(reports, callback) {
    showLoading(true);
    let host = null;
    try {
      const periodText = periodRangeLabel({ week_start: state.adminWeek || state.activePeriod?.start_date });
      const data = await buildVisualReportData(reports);
      host = document.createElement("div");
      host.innerHTML = visualReportMarkup(data, periodText);
  ...
  function frontendPlatformPaymentRule(platform, paymentRules = null, settings = null) {
  ...
  function calculateAccountPayment(views, platform, paymentRules = null, settings = null) {
    const rule = frontendPlatformPaymentRule(platform, paymentRules, settings);
  ...
  function buildFrontendPaymentBundle(reports = [], videos = [], accounts = [], paymentRules = [], settings = {}) {
    ...
    const accountRows = [...grouped.values()].map((group) => {
      const calc = calculateAccountPayment(group.views, group.platform, paymentRules, settings);
      return {
        ...group,
  ...
  function frontendPaymentRuleSummary(paymentRules = [], settings = {}) {
    const remunerated = (paymentRules || []).find((rule) => rule.pay_enabled !== false)
      || (paymentRules || []).find((rule) => rule.platform === "tiktok")
      || {};
    const rule = frontendPlatformPaymentRule(remunerated.platform || "tiktok", paymentRules, settings);
    return {
  ...
  async function loadFrontendPaymentPeriod(weekStart) {
    ...
    const bundle = buildFrontendPaymentBundle(reports, videos, accounts, paymentRules, settings || {});
    return { reports, videos, accounts, paymentRules, settings: settings || {}, ...bundle };
  }

  async function fetchAccountPayments(reportId) {
    if (!reportId) return [];
    if (reportId === state.currentReportId && state.profile?.role === "clipper") {
      ...
      const bundle = buildFrontendPaymentBundle([report], state.videos || [], state.accounts || [], paymentRules, state.settings || {});
    ...
    const bundle = await loadFrontendPaymentPeriod(weekStart);
    return bundle.accountRows.filter((row) => row.report_id === reportId);
  }

  async function fetchAdminPeriodPaymentTotals(weekStart) {
    const bundle = await loadFrontendPaymentPeriod(weekStart);
  ...
  async function adminQuickReview(payload) {
    try {
  ...
  function paymentDistributionEligible(status) {
    return String(status || "draft").toLowerCase() !== "expired";
  }

  async function fetchPaymentDistributionSettings() {
    try {
  ...
  async function savePaymentDistributionRecipient(userId) {
  ...
  function reportFinalTotal(report) {
    const distributed = state.paymentDistributionV360?.byReport?.[report?.report_id];
    return distributed ? roundMoney(distributed.final) : reportDisplayTotal(report);
  }
  ...
  function adminPlatformOverview(videos = []) {
  ...
  function principalAccountLabel(account) {
  ...
  function adminVideoCard(video, selectable = false) {
    ...
    const title = video.external_title || `Video ${video.position || ""}`;
    const urlLabel = String(video.video_url || "").replace(/^https?:\/\//i, "");
    const bucket = video.metric_review_bucket || metricReviewBucket(video);
    ...
    return `<article class="global-video-card global-video-row metric-card-${bucket}">
      ${checkbox}
      <a class="video-row-thumb" href="${esc(video.video_url)}" target="_blank" rel="noopener" title="Abrir video">${preview}<span class="video-row-platform">${platformLogo(video.platform)}</span></a>
      <div class="video-row-main"><div class="video-row-title"><a href="${esc(video.video_url)}" target="_blank" rel="noopener" title="${esc(title)}">${esc(title)}</a>${metricReviewBadge(video)}</div><p>${esc(video.clipper_name)} · ${esc(video.account_name)} · ${esc(platformLabel(video.platform))}</p><a class="video-row-url" href="${esc(video.video_url)}" target="_blank" rel="noopener" title="${esc(video.video_url)}">${esc(urlLabel)}</a></div>
  ...
  function adminLeaderVideos(videos = [], accountMap = {}) {
  ...
  async function renderAdminReports() {
    ...
    const userIds = [...new Set(reports.map(report => report.user_id).filter(Boolean))];
    await loadDistributionFlags(reportIds);
    ...
    const paymentBundle = buildFrontendPaymentBundle(reports, data.videos || [], data.accounts || [], paymentRules || [], state.settings);
    ...
    state.paymentDistributionSettingsV360 = distributionSettings || {};
    state.paymentDistributionV360 = buildPaymentDistribution(reports, state.paymentDistributionSettingsV360);
    ...
    const projected = reports.reduce((sum,report) => sum + reportFinalTotal(report),0);
    ...
    $("#content").innerHTML = `<section class="admin-command-hero admin-command-hero-v300"><div><span class="reports-kicker">RESUMEN DEL PERÍODO</span><h2>${esc(selectedPeriod?.name||periodRangeLabel(selectedPeriod)||state.adminWeek)}</h2><div class="admin-hero-stats-v300"><span><b>${reports.length}</b> cliperos</span><span><b>${data.videos.length}</b> videos</span><span><b>${num(totalViews)}</b> vistas</span><span><b>${money(projected)}</b> proyectado</span></div></div><div class="admin-home-tools-v300"><label class="period-select-v224">${uiIcon("history",14)}<select id="reportPeriodSelect">${periods.map(period=>`<option value="${period.start_date}" ${period.start_date===state.adminWeek?"selected":""}>${esc(period.name||periodRangeLabel(period))}${period.is_active?" · ACTIVO":""}</option>`).join("")}</select></label><div class="export-btn-group-v370"><button id="exportReportsBtnV300" class="btn btn-excel-v330" type="button" title="Exportar Excel">${uiIcon("report",15)}<span>Excel</span></button><button id="exportImageBtnV370" class="btn btn-secondary" type="button" title="Exportar Imagen">🖼️<span>Imagen</span></button><button id="exportPdfBtnV370" class="btn btn-secondary" type="button" title="Exportar PDF">📄<span>PDF</span></button></div></div></section>
      ${adminPlatformOverview(data.videos)}
      ${adminLeaderVideos(data.videos,accountMap)}
    ...
    $("#reportPeriodSelect")?.addEventListener("change",event=>{state.adminWeek=event.target.value;state.adminVideoData=null;renderAdminReports();});
    $("#exportReportsBtnV300")?.addEventListener("click",()=>exportWeeklyExcel(reports));
    $("#exportImageBtnV370")?.addEventListener("click",()=>exportReportImage(reports));
    $("#exportPdfBtnV370")?.addEventListener("click",()=>exportReportPdf(reports));
  ...
  function paymentFormulaNote(rows = []) {
    const sample=rows[0];
  ...
  function adminAccountPaymentBreakdown(rows = []) {
  ...
  function primaryAlias(report, accounts = []) {
  ...
  function ensureAppShell() {
    ...
    if (topActions) {
      ...
      if (!avatarButton) {
        ...
        avatarButton.addEventListener("click", () => state.profile?.role === "clipper" ? navigate("profile") : openMoreSheet());
      }
      avatarButton.innerHTML = state.profile ? avatarMarkup(state.profile, "sm") : "";
    }
    if (!$("#backTopV400")) {
  ...
  function setHeader(title, subtitle = "") {
    ...
    if(!page)return;
    if(page===state.page&&!force)return;
    state.page=page;
    state.selectedClipperId=null;
    setLocal("last_page",page);
    $("#sidebar")?.classList.remove("open");
    buildNav();
    touchPresence(true);
    await renderPage(force);
  }
  ...
  async function loadGlobalSearchData(force = false) {
    ...
    openModal(`<div class="modal-head command-head-v400"><div><span class="section-eyebrow">BUSCADOR GLOBAL</span><h2>Buscar en ClipControl</h2></div><button class="modal-close" data-search-close-v400 aria-label="Cerrar">×</button></div>
      <div class="modal-body command-body-v400"><label class="command-search-v400">${uiIcon("search",18)}<input id="globalSearchInputV400" autocomplete="off" placeholder="Nombre, usuario, TikTok, teléfono, cuenta, titular o URL…"><kbd>ESC</kbd></label><div id="globalSearchResultsV400" class="command-results-v400"><div class="command-hint-v400">Escribe al menos 2 caracteres.</div></div></div>`, "wide", layer => {
        ...
        input?.addEventListener("input",debounce(async()=>{
          ...
          box.innerHTML = `${matches.slice(0,12).map(item=>{const u=item.user,r=item.report;return `<button class="command-user-result-v400" type="button" data-search-user-v400="${esc(u.user_id)}">${avatarMarkup(u,"sm")}<span><b>${esc(`${u.names||u.username||""} ${u.surnames||""}`.trim())}</b><small>@${esc(u.username||"")} · ${item.accounts.map(a=>esc(a.account_name||a.social_alias||"")).filter(Boolean).slice(0,2).join(" · ")||"Sin cuentas"}</small></span><em>${item.videoCount} videos${r?` · ${money(reportFinalTotal(r))}`:""}</em>${uiIcon("arrow",15)}</button>`;}).join("")}${videoMatches.map(v=>`<a class="command-video-result-v400" href="${esc(v.video_url)}" target="_blank" rel="noopener">${platformLogo(v.platform)}<span><b>${esc(v.external_title||"Video")}</b><small>${num(v.views||0)} vistas · ${esc(String(v.video_url||"").replace(/^https?:\/\//,""))}</small></span>${uiIcon("arrow",15)}</a>`).join("")}` || '<div class="empty">Sin coincidencias.</div>';
  ...
  function paymentRowData(report, accounts, profileMap) {
    const mapped=state.paymentTotalsV280?.[report.report_id]||{};
    ...
    const calc=roundMoney(Number(mapped.base_pay||0)+Number(mapped.automatic_bonus_pay||0)+Number(mapped.manual_bonus_pay??report.bonus_pay??0));
    const final=roundMoney(reportFinalTotal(report));
    return {
      report,
      profile:profileMap[String(report.user_id)]||{},
      first:firstName(report),
      full:fullName(report),
      alias:primaryAlias(report,accounts),
      method:safePayMethod(report),
      account:String(report.payment_account||""),
      holder:String(report.payment_holder||fullName(report)||""),
      final,
      calculated:calc,
      base:roundMoney(Number(mapped.base_pay||0)),
      automaticBonus:roundMoney(Number(mapped.automatic_bonus_pay||0)),
      manualBonus:roundMoney(Number(mapped.manual_bonus_pay??report.bonus_pay??0)),
      discount:roundMoney(Number(dist.contribution||0)),
      adminExtra:roundMoney(Number(dist.admin_extra||0)),
  ...
  function paymentDetail(row) {
    ...
    openModal(`<div class="modal-head"><div>${avatarMarkup({...row.profile,names:row.full},"sm")}<span class="section-eyebrow">DETALLE DE PAGO</span><h2>${esc(row.full)}</h2><p>${esc(row.alias)}</p></div><button class="modal-close" data-detail-close-v400 aria-label="Cerrar">×</button></div>
  ...
  async function renderAdminPayments() {
    ...
    await loadDistributionFlags(ids);
    const userIds=[...new Set(reports.map(r=>r.user_id).filter(Boolean))];
    ...
    state.platformRuleMap=Object.fromEntries((paymentRules||[]).map(rule=>[rule.platform,rule]));
    state.settings={...(state.settings||{}),...(settings||{})};
    const bundle=buildFrontendPaymentBundle(reports,videos,accounts,paymentRules,state.settings);
    ...
    state.paymentDistributionSettingsV360=distributionSettings||{};
    state.paymentDistributionV360=buildPaymentDistribution(reports,state.paymentDistributionSettingsV360);state.profileAvatarByUserV422=Object.fromEntries((profiles||[]).map(profile=>[String(profile.id),profile]));
    ...
    const allRows=reports.map(report=>paymentRowData(report,accounts,profileMap));
    state.paymentRowsV400=allRows;
    state.paymentReportsV400=reports;
    const persisted=getLocal("payment_filters",{})||{};
    ...
    const total=roundMoney(allRows.reduce((sum,row)=>sum+row.final,0));
    ...
    const renderRows=()=>{
      ...
      $$('[data-pay-detail-v400]').forEach(button=>button.addEventListener("click",()=>{const row=allRows.find(x=>String(x.report.report_id)===String(button.dataset.payDetailV400));if(row)paymentDetail(row);}));
    };
    renderRows();
    $("#paySearchV400")?.addEventListener("input",debounce(event=>{state.paymentFiltersV400.search=event.target.value.trim();setLocal("payment_filters",state.paymentFiltersV400);renderRows();},240));
    $("#payMethodV400")?.addEventListener("change",event=>{state.paymentFiltersV400.method=event.target.value;setLocal("payment_filters",state.paymentFiltersV400);renderRows();});
    $("#paySortV400")?.addEventListener("change",event=>{state.paymentFiltersV400.sort=event.target.value;setLocal("payment_filters",state.paymentFiltersV400);renderRows();});
    $("#exportExcelV400")?.addEventListener("click",()=>exportPaymentsExcel(allRows,start));
    $("#exportImageV400")?.addEventListener("click",()=>exportReportImage(allRows,start));
    $("#exportPdfV400")?.addEventListener("click",()=>exportReportPdf(allRows,start));
  }
  ...
  function excelCell(value, style="sText", type=null) {
    const resolvedType=type||(typeof value==="number"&&Number.isFinite(value)?"Number":"String");
    return `<Cell ss:StyleID="${style}"><Data ss:Type="${resolvedType}">${resolvedType==="Number"?Number(value||0):xmlSafe(value)}</Data></Cell>`;
  }

  function excelSheet(name, rows, widths=[]) {
    return `<Worksheet ss:Name="${xmlSafe(name)}"><Table>${widths.map(w=>`<Column ss:AutoFitWidth="0" ss:Width="${w}"/>`).join("")}${rows.map(row=>`<Row>${row.join("")}</Row>`).join("")}</Table><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><FreezePanes/><FrozenNoSplit/><SplitHorizontal>1</SplitHorizontal><TopRowBottomPane>1</TopRowBottomPane><ProtectObjects>False</ProtectObjects><ProtectScenarios>False</ProtectScenarios></WorksheetOptions></Worksheet>`;
  }

  async function exportPaymentsExcel(rows, start) {
    showLoading(true);
    try {
      ...
      const payments=[
        ["Foto / referencia","Nombre","Seudónimo","Método","Cuenta","Titular","Pago final"].map(v=>excelCell(v,"sHeader")),
        ...sorted.map(row=>[
          excelCell(row.profile.avatar_url||`Iniciales ${row.full.split(/\s+/).slice(0,2).map(x=>x[0]?.toUpperCase()).join("")}`,"sText"),
          excelCell(row.full,"sText"),
          excelCell(row.alias,"sText"),
          excelCell(row.method,"sText"),
          excelCell(row.account||"Pendiente","sText"),
          excelCell(row.holder||"—","sText"),
          excelCell(row.final,"sMoney","Number")
        ])
      ];
      const withPay=sorted.filter(r=>r.final>0).length;
      const total=roundMoney(sorted.reduce((sum,r)=>sum+r.final,0));
      const summary=[
        ["Indicador","Valor"].map(v=>excelCell(v,"sHeader")),
        [excelCell("Personas"),excelCell(sorted.length,"sNumber","Number")],
        [excelCell("Con pago"),excelCell(withPay,"sNumber","Number")],
        [excelCell("Sin pago"),excelCell(sorted.length-withPay,"sNumber","Number")],
        [excelCell("Total"),excelCell(total,"sMoney","Number")],
        [excelCell("Período"),excelCell(periodRangeLabel({week_start:start}))],
        [excelCell("Fecha de generación"),excelCell(dateTimeLabel(new Date().toISOString()))],
      ];
      const detail=[
        ["Clipero","Pago calculado","Descuento","Pago final","Distribución administrativa"].map(v=>excelCell(v,"sHeader")),
        ...sorted.map(row=>[excelCell(row.full),excelCell(row.calculated,"sMoney","Number"),excelCell(row.discount,"sMoney","Number"),excelCell(row.final,"sMoney","Number"),excelCell(row.isRecipient?"Receptor administrativo":row.discount?`${money(row.discount)} descontado + ${money(row.adminExtra)} adicional`:"No aplica")])
      ];
      const workbook=`<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:x="urn:schemas-microsoft-com:office:excel">${styles}${excelSheet("PAGOS",payments,[115,150,125,90,125,160,95])}${excelSheet("RESUMEN",summary,[160,180])}${excelSheet("DETALLE INTERNO",detail,[170,105,95,105,220])}</Workbook>`;
      const blob=new Blob(["\ufeff",workbook],{type:"application/vnd.ms-excel"});
  ...
  function visualRowsMarkup(rows) {
    return rows.map(row=>`<tr><td><div class="export-person-v400">${avatarMarkup({...row.profile,names:row.full},"sm")}<b>${esc(row.first)}</b></div></td><td>${esc(row.alias)}</td><td><span>${esc(row.method)}</span></td><td>${esc(row.account||"Pendiente")}</td><td>${esc(row.holder||"—")}</td><td class="export-pay-v400">${money(row.final)}</td></tr>`).join("");
  }

  function visualReportMarkup(rows, start, compact=false, summaryRows=null) {
    const sorted=[...rows].sort((a,b)=>a.final-b.final||a.full.localeCompare(b.full,"es"));
    const summary=[...(summaryRows||rows)];
    const withPay=summary.filter(r=>r.final>0).length;
    const total=roundMoney(summary.reduce((sum,r)=>sum+r.final,0));
    return `<div class="export-report-v400 ${compact?"pdf-page-v400":""}"><header><div><span>CLIPCONTROL</span><h1>PAGOS ACTUALIZADOS · TODOS LOS CLIPEROS</h1><p>Pago final por persona · ${esc(periodRangeLabel({week_start:start}))}</p></div><b>${money(total)}</b></header><section class="export-kpis-v400"><div><small>PERSONAS</small><strong>${summary.length}</strong></div><div><small>CON PAGO</small><strong>${withPay}</strong></div><div><small>CON S/0</small><strong>${summary.length-withPay}</strong></div><div><small>TOTAL GENERAL</small><strong>${money(total)}</strong></div></section><div class="export-table-v400"><table><thead><tr><th>PRIMER NOMBRE</th><th>SEUDÓNIMO</th><th>MÉTODO</th><th>CUENTA DE PAGO</th><th>TITULAR</th><th>PAGO FINAL</th></tr></thead><tbody>${visualRowsMarkup(sorted)||'<tr><td colspan="6">Sin registros.</td></tr>'}</tbody></table></div><footer>Generado por ClipControl · ${esc(dateTimeLabel(new Date().toISOString()))}</footer></div>`;
  }

  async function captureVisual(markup) {
    await ensureExportLibs();
  ...
  async function exportReportPdf(rows,start) {
    ...
    try {
      ...
      for(let i=0;i<chunks.length;i++){
        ...
        const canvas=await captureVisual(visualReportMarkup(chunks[i],start,true,sorted));
  ...
  function renderProfilePage() {
    ...
    cancel?.addEventListener("click",()=>{pending=null;if(pendingUrl)URL.revokeObjectURL(pendingUrl);pendingUrl=null;fileInput.value="";preview.innerHTML=avatarMarkup(p,"xl");save.classList.add("hidden");cancel.classList.add("hidden");});
    $("#removeAvatarBtnV400")?.addEventListener("click",removeMyAvatar);
    $("#profileForm")?.addEventListener("submit",saveOwnProfile);
    $("#paymentProfileForm")?.addEventListener("submit",async event=>{event.preventDefault();const f=Object.fromEntries(new FormData(event.target));await saveProfile({names:p.names,surnames:p.surnames,phone:p.phone,primary_social_url:p.primary_social_url,...f});});
  }

  async function renderAdminSettingsBase() {
    await renderAdminSettingsCore();
    const pane=$("[data-settings-pane='payments']");
    if(!pane)return;
    const [config,users]=await Promise.all([fetchPaymentDistributionSettings(),query(state.supabase.from("admin_clipper_overview").select("user_id,role,active,username,names,surnames").eq("role","clipper").order("username")).catch(()=>[])]);
  ...
  function initAppShell() {
    ...
    document.addEventListener("keydown",event=>{
      ...
      if(!typing&&event.key==="/"){event.preventDefault();openGlobalSearch();}
    });
    if("serviceWorker" in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=4.4.1-20260924").catch(()=>null),{once:true});}
  }
  ...
  async function loadDistributionFlags(reportIds = []) {
  ...
  async function preloadDistributionFlagsForWeek(weekStart) {
    if (!weekStart) return;
    try {
  ...
  function reportDistributionExempt(report) {
  ...
  function buildPaymentDistribution(reports = [], settings = null) {
    ...
    const discount = Math.max(0, roundMoney(config.minimum_contribution ?? 30));
    const adminExtra = discount;

    const rows = source.map(report => {
      const original = roundMoney(reportDisplayTotal(report));
      const isRecipient = String(report.user_id || "") === recipientId;
      const exempt = reportDistributionExempt(report);
      const eligibleStatus = paymentDistributionEligible(report.status);
      ...
      const contribution = deductionEligible ? roundMoney(Math.min(original, discount)) : 0;
      return {
        report_id: report.report_id,
        user_id: report.user_id,
        original,
        ...
        final: roundMoney(original - contribution),
    ...
    const pool = roundMoney(rows.reduce((sum, row) => sum + row.contribution + row.admin_extra, 0));
    const validCount = rows.filter(row => row.counted_valid).length;
    const recipient = rows.find(row => row.is_recipient);
    if (enabled && recipient) recipient.final = roundMoney(recipient.original + pool);
    const originalTotal = roundMoney(rows.reduce((sum, row) => sum + row.original, 0));
    const finalTotal = roundMoney(rows.reduce((sum, row) => sum + row.final, 0));
  ...
  function clipperFinalPayment(basePay, summary, settings) {
    const amount = roundMoney(basePay);
    if (reportDistributionExempt(summary)) return amount;
    const cfg = settings || {};
    const isRecipient = String(cfg.recipient_user_id || "") === String(state.profile?.id || "");
    const eligibleStatus = paymentDistributionEligible(summary?.status);
    if (cfg.enabled !== false && cfg.recipient_user_id && !isRecipient && eligibleStatus && amount > 0) {
      return roundMoney(Math.max(0, amount - Math.max(0, Number(cfg.minimum_contribution ?? 30))));
    }
    return amount;
  }

  function clipperAccountProgress(rows = []) {
  ...
  async function uploadAvatarOnly(file) {
  ...
  function openProfileModal(required = false) {
    ...
    openModal(`<div class="modal-head"><div><span class="section-eyebrow">PERFIL</span><h2>${title}</h2><p>${subtitle}</p></div>${required ? "" : '<button id="profileX" class="modal-close">×</button>'}</div>
      <form id="modalProfileFormV410"><div class="modal-body onboarding-v410">
        <section class="onboarding-photo-v410"><div id="onboardingAvatarV410">${avatarMarkup(p,"lg")}</div><div><h3>Foto de perfil</h3><p>Usa una foto reconocible de tu cuenta principal.</p><label class="btn btn-secondary" for="onboardingAvatarFileV410">${uiIcon("upload",15)} ${p.avatar_url?"Cambiar foto":"Subir foto"}</label><input id="onboardingAvatarFileV410" type="file" hidden accept="image/jpeg,image/png,image/webp"><small>${needsPhoto?"Obligatoria para continuar":"Foto registrada"}</small></div></section>
        ...
        $("#modalProfileFormV410",layer)?.addEventListener("submit",async event=>{
          ...
          try{
            ...
            if(file){const avatarUrl=await uploadAvatarOnly(file);state.profile.avatar_url=avatarUrl;}
  ...
  function topVideoPreview(video) {
  ...
  async function renderAdminDashboard() {
    ...
    await loadDistributionFlags(reports.map(r=>r.report_id));
    ...
    const projected=roundMoney(reports.reduce((sum,r)=>sum+reportFinalTotal(r),0));
    ...
    $("#dashboardPeriodV400")?.addEventListener("change",event=>{state.adminWeek=event.target.value;state.adminVideoData=null;renderAdminDashboard();});
    $$('[data-dashboard-go-v410]').forEach(b=>b.addEventListener("click",()=>navigate(b.dataset.dashboardGoV410)));
    $$('[data-quick-v410]').forEach(button=>button.addEventListener("click",()=>{const action=button.dataset.quickV410;if(action==="search")openGlobalSearch();else navigate(action);}));
    bindAdminReportButtons();animateDynamicNumbers($("#content"));
  }

  function renderAdminRulesTab(profile,rules) {
    renderAdminRulesBase(profile,rules);
  ...
  function reportAccountFilters(videos,accounts,selected="all") {
  ...
  async function openAdminReportDetail(reportId) {
    ...
    try{
      ...
      let [videos,accounts,observations,rules,accountPayRows]=await Promise.all([
        ...
        query(state.supabase.from("platform_payment_rules").select("*")),fetchAccountPayments(reportId)
      ]);
  ...
  function suggestionPlatformFromUrl(value) {
  ...
  function suggestionPreview(link) {
    const url=String(link?.url||""), platform=link?.platform||suggestionPlatformFromUrl(url)||"";
    let embed="";
  ...
  function suggestionStatusBadge(status) { const cls=status==="used"||status==="accepted"?"pill-green":status==="dismissed"?"pill-red":status==="reviewing"?"pill-yellow":"chip";return `<span class="pill ${cls}">${esc(SUGGESTION_STATUS_V410[status]||status)}</span>`; }

  async function fetchSuggestions(admin=false) {
  ...
  function suggestionCard(item,admin=false) {
    const owner=item.profile, title=item.title||SUGGESTION_KIND_V410[item.kind]||"Idea";
    return `<article class="idea-card-v410"><header>${admin&&owner?`${avatarMarkup(owner,"sm")}<div><strong>${esc(`${owner.names||owner.username||""} ${owner.surnames||""}`.trim())}</strong><small>@${esc(owner.username||"")}</small></div>`:`<div><strong>${esc(title)}</strong><small>${esc(SUGGESTION_KIND_V410[item.kind]||item.kind)}</small></div>`}<div>${suggestionStatusBadge(item.status)}</div></header>${admin?`<h3>${esc(title)}</h3>`:""}${item.message?`<p class="idea-message-v410">${esc(item.message)}</p>`:""}${item.reaction_angle?`<div class="idea-angle-v410"><b>Enfoque sugerido</b><p>${esc(item.reaction_angle)}</p></div>`:""}<div class="idea-links-v410">${(item.links||[]).map(suggestionPreview).join("")}</div><footer><small>${dateTimeLabel(item.created_at)}</small>${admin?`<div class="idea-admin-actions-v410"><select data-idea-status-v410="${item.id}">${Object.entries(SUGGESTION_STATUS_V410).map(([value,label])=>`<option value="${value}" ${item.status===value?"selected":""}>${label}</option>`).join("")}</select><button class="btn btn-secondary btn-sm" data-save-idea-v410="${item.id}">Guardar</button></div>`:item.admin_note?`<span>${esc(item.admin_note)}</span>`:""}</footer></article>`;
  }
  ...
  async function renderAdminIdeas() {
    ...
    const items=await fetchSuggestions(true);state.ideaFiltersV410=state.ideaFiltersV410||{kind:"all",status:"all"};const f=state.ideaFiltersV410;
  ...
  function openMoreSheet() {
    ...
    openModal(`<div class="modal-head"><div><span class="section-eyebrow">MÁS</span><h2>${admin?"Centro administrativo":"Tu espacio"}</h2><p>Accesos y configuración.</p></div><button class="modal-close" data-close-v420>×</button></div><div class="modal-body more-sheet-v400 more-sheet-v420">${items.map(([page,icon,label])=>`<button type="button" data-more-page-v420="${page}">${uiIcon(icon,19)}<span>${label}</span>${uiIcon("arrow",15)}</button>`).join("")}<button type="button" data-more-action-v420="notices">${uiIcon("megaphone",19)}<span>${admin?"Notificaciones":"Avisos"}</span>${uiIcon("arrow",15)}</button><button type="button" data-more-action-v420="theme">${uiIcon("activity",19)}<span>Tema</span>${uiIcon("arrow",15)}</button><div class="more-profile-v400">${avatarMarkup(state.profile,"sm")}<div><b>${esc(`${state.profile.names||""} ${state.profile.surnames||""}`.trim()||state.profile.username)}</b><small>@${esc(state.profile.username||"")}</small></div></div><button type="button" class="danger" data-more-action-v420="logout">${uiIcon("logout",19)}<span>Cerrar sesión</span></button></div>`,"small",layer=>{
  ...
  function addSuggestionLinkRow(container,index) {
  ...
  function openSuggestionComposer(kind="reaction") {
    ...
    openModal(`<div class="modal-head modal-head-v420"><div><span class="section-eyebrow">NUEVA IDEA</span><h2>${reaction?"Contenido para reaccionar":"Sugerencia de gestión"}</h2><p>${reaction?"Comparte publicaciones que valga la pena responder.":"Envía una propuesta concreta y constructiva."}</p></div><button class="modal-close" data-close-idea-v420>×</button></div><form id="suggestionFormV420" class="modal-form-v420"><div class="modal-body suggestion-form-v420 compact-suggestion-v420"><label>Título <small>opcional</small><input name="title" maxlength="160" placeholder="${reaction?"Tema o contexto":"Título de la sugerencia"}"></label>${reaction?'<div class="suggestion-two-v420"><label>Comentario general<textarea name="message" rows="2" maxlength="2000" placeholder="¿Qué está pasando y por qué importa?"></textarea></label><label>Enfoque general<textarea name="reaction_angle" rows="2" maxlength="2000" placeholder="Cómo podría responderse"></textarea></label></div>':'<label>Sugerencia<textarea name="message" rows="4" maxlength="3000" required placeholder="Describe tu propuesta"></textarea></label>'}<div class="suggestion-links-title-v420"><div><h3>${reaction?"Publicaciones":"Referencias"}</h3><p>${reaction?"Agrega uno o varios links.":"Opcional: agrega links de referencia."}</p></div><button id="addSuggestionLinkV420" class="btn btn-gold btn-sm" type="button">${uiIcon("plus",14)} Agregar link</button></div><div id="suggestionLinksV420" class="suggestion-link-list-v420"></div></div><div class="modal-foot"><button type="button" class="btn btn-ghost" data-close-idea-v420>Cancelar</button><button class="btn btn-gold">Enviar idea</button></div></form>`,"medium",layer=>{
      const container=$("#suggestionLinksV420",layer); if(reaction)addSuggestionLinkRow(container,1);
      $$('[data-close-idea-v420]',layer).forEach(b=>b.addEventListener("click",closeModal));
      $("#addSuggestionLinkV420",layer)?.addEventListener("click",()=>{const count=container.querySelectorAll('[data-suggestion-link-row]').length;if(count>=10)return toast("Máximo 10 links por envío.","error");addSuggestionLinkRow(container,count+1);});
      $("#suggestionFormV420",layer)?.addEventListener("submit",async event=>{event.preventDefault();const f=Object.fromEntries(new FormData(event.target)),rows=[...container.querySelectorAll('[data-suggestion-link-row]')],links=rows.map((row,index)=>({url:row.querySelector('[name="idea_url"]')?.value.trim()||"",why:row.querySelector('[name="idea_why"]')?.value.trim()||"",note:row.querySelector('[name="idea_note"]')?.value.trim()||"",position:index+1})).filter(x=>x.url);if(reaction&&!links.length)return toast("Agrega al menos un link.","error");for(const link of links){if(!suggestionPlatformFromUrl(link.url))return toast("Solo se aceptan links de TikTok, Instagram, YouTube o Facebook.","error");}showLoading(true);try{await query(state.supabase.rpc("clipcontrol_submit_suggestion",{p_kind:kind,p_title:f.title||null,p_message:f.message||null,p_reaction_angle:f.reaction_angle||null,p_links:links}));closeModal();toast("Idea enviada","success");if(state.page==="ideas")await renderClipperIdeas();refreshNavBadges();}catch(error){toast(errorMessage(error),"error");}finally{showLoading(false);}});
    });
  }

  async function renderClipperIdeas() {
    setHeader("Ideas","Contenido y sugerencias");
    const items=await fetchSuggestions(false);
    $("#content").innerHTML=`<section class="ideas-hero-v420"><div><span class="section-eyebrow">IDEAS</span><h2>Comparte lo que puede funcionar</h2><p>Contenido para reaccionar y propuestas de mejora, en un solo lugar.</p></div><div><button class="btn btn-gold" data-new-idea-v420="reaction">🎬 Sugerir contenido</button><button class="btn btn-secondary" data-new-idea-v420="management">💡 Sugerencia</button></div></section><section class="card compact-card"><div class="section-title-row"><div><span class="section-eyebrow">MIS ENVÍOS</span><h3>${items.length} idea${items.length===1?"":"s"}</h3></div></div><div class="ideas-grid-v410">${items.map(item=>suggestionCard(item,false)).join("")||'<div class="empty empty-v420"><b>Sin ideas todavía</b><span>Cuando encuentres algo interesante, envíalo desde aquí.</span></div>'}</div></section>`;
    $$('[data-new-idea-v420]').forEach(b=>b.addEventListener("click",()=>openSuggestionComposer(b.dataset.newIdeaV420)));
  }
  ...
  async function fetchMyFinalPayment(report) {
    ...
    let final = null;
    try {
      const [rows,settings] = await Promise.all([fetchAccountPayments(report.report_id),fetchPaymentDistributionSettings()]);
      await loadDistributionFlags([report.report_id]);
      const raw=roundMoney(rows.reduce((sum,row)=>sum+Number(row.final_pay||0),0)+Number(report.bonus_pay||0));
      if(String(settings?.recipient_user_id||"")!==String(state.profile?.id||"")) final=clipperFinalPayment(raw,report,settings);
    } catch (_) {}
    if(final===null){
  ...
  async function renderClipperHistory() {
    ...
    const payments=await Promise.all((reports||[]).map(r=>fetchMyFinalPayment(r)));
  ...
  async function openReportDetail(reportId, adminMode=false) {
    ...
    try{
      ...
      const [videos,accounts,finalPay]=await Promise.all([
        query(state.supabase.from("videos").select("*").eq("report_id",reportId).is("deleted_at",null).order("position")),
        query(state.supabase.from("social_accounts").select("*").eq("user_id",summary.user_id).order("platform")),
        adminMode?Promise.resolve(reportFinalTotal(summary)):fetchMyFinalPayment(summary)
      ]);
  ...
  async function fetchSupportTickets() {
    ...
    if(!isAdmin())q=q.eq("user_id",state.profile.id);
  ...
  function supportStatusBadge(status){const cls=status==="closed"||status==="resolved"?"pill-green":status==="waiting_user"?"pill-yellow":status==="reviewing"?"chip":"pill-red";return `<span class="pill ${cls}">${esc(SUPPORT_STATUS_V420[status]||status)}</span>`;}
  ...
  function openNewSupportTicket(){
    openModal(`<div class="modal-head"><div><span class="section-eyebrow">SOPORTE</span><h2>Nuevo ticket</h2><p>Cuéntanos qué necesitas y quedará registrado.</p></div><button class="modal-close" data-support-close-v420>×</button></div><form id="supportNewFormV420" class="modal-form-v420"><div class="modal-body support-form-v420"><div class="form-grid compact-form"><label>Categoría<select name="category" required>${Object.entries(SUPPORT_CATEGORY_V420).map(([v,l])=>`<option value="${v}">${l}</option>`).join("")}</select></label><label>Prioridad<select name="priority"><option value="normal">Normal</option><option value="high">Alta</option><option value="low">Baja</option></select></label><label class="full">Asunto<input name="subject" maxlength="180" required placeholder="Resumen breve"></label><label class="full">Mensaje<textarea name="message" rows="5" maxlength="8000" required placeholder="Describe el problema con el mayor detalle posible"></textarea></label></div></div><div class="modal-foot"><button type="button" class="btn btn-ghost" data-support-close-v420>Cancelar</button><button class="btn btn-gold">Crear ticket</button></div></form>`,"small",layer=>{
      ...
      $("#supportNewFormV420",layer)?.addEventListener("submit",async event=>{event.preventDefault();const f=Object.fromEntries(new FormData(event.target));showLoading(true);try{await query(state.supabase.rpc("clipcontrol_create_support_ticket_v420",{p_category:f.category,p_subject:f.subject,p_message:f.message,p_priority:f.priority}));closeModal();toast("Ticket creado","success");await renderSupport();refreshNavBadges();}catch(error){toast(errorMessage(error),"error");}finally{showLoading(false);}});
    });
  }
  ...
  async function renderSupport(){
    setHeader("Soporte",isAdmin()?"Mesa de ayuda":"Ayuda y seguimiento");
    const tickets=await fetchSupportTickets();
    const openCount=tickets.filter(t=>!["resolved","closed"].includes(t.status)).length;
    $("#content").innerHTML=`<section class="support-hero-v420"><div><span class="section-eyebrow">MESA DE AYUDA</span><h2>${isAdmin()?`${openCount} ticket${openCount===1?"":"s"} activo${openCount===1?"":"s"}`:"¿Necesitas ayuda?"}</h2><p>${isAdmin()?"Organiza y responde consultas sin perderlas en otros canales.":"Crea un ticket y sigue la respuesta desde ClipControl."}</p></div>${!isAdmin()?'<button id="newSupportTicketV420" class="btn btn-gold">🎫 Nuevo ticket</button>':""}</section><div class="support-list-v420">${tickets.map(t=>`<button class="support-ticket-card-v420" data-ticket-v420="${t.id}"><div class="support-ticket-icon-v420">${t.category==="payment"?"S/":t.category==="metrics"?"↻":t.category==="technical"?"⚙":"?"}</div><div><header><strong>${esc(t.subject)}</strong>${supportStatusBadge(t.status)}</header><p>${isAdmin()&&t.profile?`${esc(`${t.profile.names||t.profile.username||""} ${t.profile.surnames||""}`.trim())} · `:""}${esc(SUPPORT_CATEGORY_V420[t.category]||t.category)}</p><small>Actualizado ${dateTimeLabel(t.updated_at)}</small></div><span>${uiIcon("arrow",17)}</span></button>`).join("")||'<div class="empty card empty-v420"><b>No hay tickets</b><span>Todo está en orden por aquí.</span></div>'}</div>`;
    $("#newSupportTicketV420")?.addEventListener("click",openNewSupportTicket);$$('[data-ticket-v420]').forEach(b=>b.addEventListener("click",()=>openSupportTicket(b.dataset.ticketV420)));
  }
  ...
  async function renderAdminSettings(){
    ...
    $("#brandingFormV420")?.addEventListener("submit",async e=>{e.preventDefault();const f=Object.fromEntries(new FormData(e.currentTarget));showLoading(true);try{let logo=state.settings.brand_logo_url||null;if(pendingLogo)logo=await uploadBrandLogo(pendingLogo);const updated=await query(state.supabase.rpc("admin_update_branding_v420",{p_brand_name:f.brand_name,p_brand_tagline:f.brand_tagline,p_brand_logo_url:logo}));state.settings=Array.isArray(updated)?updated[0]:updated;applyBranding();toast("Marca actualizada","success");await renderAdminSettings();}catch(error){toast(errorMessage(error),"error");}finally{showLoading(false);if(pendingUrl)URL.revokeObjectURL(pendingUrl);}});
  }

  async function renderAdminPage(){
    const week=state.adminWeek||state.activePeriod?.start_date||currentWeekStartISO();
    if(["dashboard","reports","payments"].includes(state.page))await preloadDistributionFlagsForWeek(week);
    ...
    if(state.page==="ideas")return renderAdminIdeas();
    if(state.page==="support")return renderSupport();
    if(state.page==="profile")return renderAdminProfile();
    if(state.page==="announcements")return renderAdminAnnouncements();
    if(state.page==="settings")return renderAdminSettings();
    state.page="dashboard";return renderAdminDashboard();
  }


  // Menú responsive: un único listener de botón y cierre por navegación.
  window.addEventListener("resize",()=>{if(window.innerWidth>1180)closeSidebar();},{passive:true});

  // Herramientas de diagnóstico solo cuando debug=true en supabase-config.js.
  if (window.CLIPCONTROL_SUPABASE?.debug === true) {
    window.clipcontrolDebugUI = () => ({version:CLIPCONTROL_FRONTEND_VERSION, ui:document.documentElement.dataset.clipcontrolUi, page:state.page, role:state.profile?.role, nativePush:nativePushSnapshot()});
    ...
    window.clipcontrolDebugFrontend = () => ({
      ...
      source: "app-v4.4.1-clean-push-core.js",
  ...
  /* ===================================================================
     CLIPCONTROL 4.4 · CLEAN MOBILE CORE
     - Navegación no bloqueada por presence RPC.
     - Cache corto de contexto global y datos del clipero.
     - Realtime fuerza refresco de datos; navegación reutiliza cache reciente.
     - Evita reconstrucciones y consultas repetidas en cada toque móvil.
     =================================================================== */

  const CC_PERF = {
    globalTtlMs: 90_000,
    clipperTtlMs: 15_000,
    accountsTtlMs: 120_000,
    rulesTtlMs: 300_000,
    reportIdTtlMs: 60_000,
    observationsTtlMs: 120_000,
    accessTtlMs: 120_000,
    announcementTtlMs: 30_000,
  };

  function ccMobilePerfMode() {
    const coarse = window.matchMedia?.("(pointer: coarse)")?.matches;
    return Boolean(coarse || window.innerWidth <= 1180);
  }

  function applyMobilePerfMode() {
    document.documentElement.classList.toggle("cc-mobile-perf", ccMobilePerfMode());
  }

    async function loadGlobalContext(force=false) {
    const now=Date.now();
    if(!force&&state._globalContextLoadedAt&&now-state._globalContextLoadedAt<CC_PERF.globalTtlMs&&state.settings&&state.activePeriod)return;
    if(state._globalContextInflight)return state._globalContextInflight;
    state._globalContextInflight=(async()=>{
      ...
      const previousPeriodId=state.activePeriod?.id||null;
      state.settings=settings;
      state.activePeriod=periods?.[0]||null;
      if(previousPeriodId&&state.activePeriod?.id&&previousPeriodId!==state.activePeriod.id){
        state.currentReportId=null;
        state._reportIdLoadedAt=0;
        state._clipperDataLoadedAt=0;
      }
      ...
      state._globalContextLoadedAt=Date.now();
    })();
    try{await state._globalContextInflight;}finally{state._globalContextInflight=null;}
  }

    async function loadClipperCurrentData(force=false) {
    const now=Date.now();
    const hasDynamic=Boolean(state.currentReportId&&state.currentSummary&&Array.isArray(state.videos));
    if(!force&&hasDynamic&&state._clipperDataLoadedAt&&now-state._clipperDataLoadedAt<CC_PERF.clipperTtlMs)return;
    if(state._clipperDataInflight)return state._clipperDataInflight;

    state._clipperDataInflight=(async()=>{
      if(!state.autoSubmitFallbackChecked){
        state.autoSubmitFallbackChecked=true;
        state.supabase.rpc("clipcontrol_auto_submit_due_reports_v234").then(()=>{}).catch(()=>{});
      }

      const accountsPromise=(state._accountsLoadedAt&&Date.now()-state._accountsLoadedAt<CC_PERF.accountsTtlMs&&Array.isArray(state.accounts))
      ...
      const rulesPromise=(state._rulesLoadedAt&&Date.now()-state._rulesLoadedAt<CC_PERF.rulesTtlMs&&state.platformRuleMap)
        ? Promise.resolve(Object.values(state.platformRuleMap))
        : query(state.supabase.from("platform_payment_rules").select("*")).catch(()=>[]);
      const reportIdPromise=(state.currentReportId&&state._reportIdLoadedAt&&Date.now()-state._reportIdLoadedAt<CC_PERF.reportIdTtlMs)
        ? Promise.resolve(state.currentReportId)
        : query(state.supabase.rpc("ensure_weekly_report",{p_week_start:null}));
      const accessPromise=(!state._accessPaymentLoadedAt||Date.now()-state._accessPaymentLoadedAt>CC_PERF.accessTtlMs)
        ? refreshMyAccessPayment().catch(()=>null)
        : Promise.resolve(state.accessPayment);

      // Acceso/pago corre en paralelo, pero no retrasa el inicio de las consultas del reporte.
      const [accounts,rulesResult,reportId]=await Promise.all([accountsPromise,rulesPromise,reportIdPromise]);
      state.accounts=accounts||[];
      state._accountsLoadedAt=Date.now();
      state.platformRuleMap=Object.fromEntries((rulesResult||[]).map(rule=>[rule.platform,rule]));
      state._rulesLoadedAt=Date.now();
      state.currentReportId=reportId;
      state._reportIdLoadedAt=Date.now();
      if(!reportId){
        state.currentSummary=null;state.videos=[];state.observations=[];state.platformSummary=[];
        state._clipperDataLoadedAt=Date.now();return;
      }

      const sameObservationReport=state._observationsReportId===reportId;
      const observationsPromise=(sameObservationReport&&state._observationsLoadedAt&&Date.now()-state._observationsLoadedAt<CC_PERF.observationsTtlMs)
        ? Promise.resolve(state.observations||[])
        : query(state.supabase.from("report_observations").select("*").eq("report_id",reportId).order("created_at",{ascending:false}));

      const [summary,videos,platformSummary,observations]=await Promise.all([
      ...
      state._observationsReportId=reportId;
      state._observationsLoadedAt=Date.now();
      state._clipperDataLoadedAt=Date.now();
      try{await accessPromise;state._accessPaymentLoadedAt=Date.now();}catch(_){}
    })();
    try{await state._clipperDataInflight;}finally{state._clipperDataInflight=null;}
  }

    function buildNav() {
    if(!state.profile)return;
    const admin=["admin","superadmin"].includes(state.profile.role);
    const roleKey=admin?"admin":"clipper";
    const desktop=admin
    ...
    const nav=$("#nav");
    if(nav){
      if(nav.dataset.roleKey!==roleKey){
        nav.dataset.roleKey=roleKey;
        nav.innerHTML=desktop.map(([id,icon,label])=>`<button data-page="${id}">${navIcon(icon)}<span>${label}</span></button>`).join("");
        if(nav.dataset.bound!=="1"){
          nav.dataset.bound="1";
          nav.addEventListener("click",event=>{
            const button=event.target.closest?.("[data-page]");
            if(button&&nav.contains(button))navigate(button.dataset.page);
          });
        }
      }
      $$('[data-page]',nav).forEach(button=>button.classList.toggle("active",button.dataset.page===state.page));
    }

    const mobileNav=$("#mobileNav");
    if(mobileNav){
      if(mobileNav.dataset.roleKey!==roleKey){
        mobileNav.dataset.roleKey=roleKey;
        mobileNav.innerHTML=mobile.map(([id,icon,label])=>`<button data-mobile-page="${id}" aria-label="${esc(label)}" class="${id==="__add"?"mobile-add":""}">${navIcon(icon)}${id==="__add"?"":`<span>${label}</span>`}</button>`).join("");
        if(mobileNav.dataset.bound!=="1"){
          mobileNav.dataset.bound="1";
          mobileNav.addEventListener("click",event=>{
            ...
            navigate(button.dataset.mobilePage);
          });
        }
      }
      $$('[data-mobile-page]',mobileNav).forEach(button=>button.classList.toggle("active",button.dataset.mobilePage===state.page));
    }
  }
    ...
    try{
      const isAdmin=["admin","superadmin"].includes(state.profile.role);
      const clipperNeedsData=!isAdmin&&["dashboard","videos","networks","channel"].includes(state.page);
      const tasks=[loadGlobalContext(false)];
      if(clipperNeedsData)tasks.push(loadClipperCurrentData(force));
      await Promise.all(tasks);
      if(isAdmin)await renderAdminPage();else await renderClipperPage();
      buildNav();
      animateDynamicNumbers(content);
      // No bloquea cada navegación con otra consulta.
      refreshAnnouncementBadge(false);
    }catch(error){
      console.error(error);
      if(content)content.innerHTML=`<div class="alert alert-danger"><div>⚠️</div><div><strong>No se pudo cargar esta sección</strong><p>${esc(errorMessage(error))}</p></div></div>`;
      toast(errorMessage(error),"error");
    ...
    if(state.page==="ideas")return renderClipperIdeas();
    if(state.page==="support")return renderSupport();
    if(state.page==="history")return renderClipperHistory();
    if(state.page==="profile")return renderProfilePage();
    state.page="dashboard";return renderClipperDashboard();
  }
    ...
    state.liveRefreshTimer=setTimeout(async()=>{
      ...
      try{
        if(state.profile.role==="clipper"&&["dashboard","videos"].includes(state.page)){
          state._clipperDataLoadedAt=0;
          await loadClipperCurrentData(true);
          await renderLiveSilently(async()=>{
            if(state.page==="dashboard")await renderClipperDashboard();
            else renderClipperVideos();
          });
        }else if(["admin","superadmin"].includes(state.profile.role)&&(state.page==="dashboard"||state.page==="reports")){
          await loadGlobalContext(table==="reporting_periods");
          await renderLiveSilently(async()=>{
            if(state.page==="dashboard")await renderAdminDashboard();
            else await renderAdminReports();
          });
        }
  ...
  window.addEventListener("resize",applyMobilePerfMode,{passive:true});
